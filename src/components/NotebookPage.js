        ...
        <div class="p-8 h-full flex flex-col relative bg-transparent overflow-hidden">
            ...
            <header class="flex justify-between items-center mb-8 shrink-0">
                 <div class="flex items-center gap-4">
                    ...
                    <h1 class="text-3xl font-bold text-dark font-handwriting">${t('digital_notebook')}</h1>
                    ...
                    <div class="ml-6 pl-6 border-l border-gray-300/50 flex items-center ${activeNote ? '' : 'opacity-50 pointer-events-none'} transition-opacity relative">
                         ...
                         <div class="flex items-center bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden divide-x divide-gray-100">
                             <!-- Style Buttons -->
                             <button class="toolbar-btn w-9 h-9 flex items-center justify-center hover:bg-gray-50 text-gray-500 hover:text-dark transition-all" data-cmd="bold" title="${t('bold')}"><b class="font-serif">B</b></button>
                             <button class="toolbar-btn w-9 h-9 flex items-center justify-center hover:bg-gray-50 text-gray-500 hover:text-dark transition-all italic" data-cmd="italic" title="${t('italic')}"><i class="font-serif">I</i></button>
                             <button class="toolbar-btn w-9 h-9 flex items-center justify-center hover:bg-gray-50 text-gray-500 hover:text-dark transition-all underline" data-cmd="underline" title="${t('underline')}"><u class="font-serif">U</u></button>
                             
                             <!-- Color Picker -->
                             <button id="btn-color-picker" class="toolbar-btn w-9 h-9 flex items-center justify-center hover:bg-gray-50 text-gray-500 hover:text-dark transition-all" title="${t('text_color')}">
                             ...
                             <!-- Highlight Picker -->
                             <button id="btn-highlight-picker" class="toolbar-btn w-9 h-9 flex items-center justify-center hover:bg-gray-50 text-gray-500 hover:text-dark transition-all" title="${t('highlight_color')}">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                             ...
                             <!-- Align Button -->
                             <button class="toolbar-btn w-9 h-9 flex items-center justify-center hover:bg-gray-50 text-gray-500 hover:text-dark transition-all" data-cmd="cycleAlign" data-state="left" title="${t('align_text')}">
                 ...
                 <div class="flex items-center gap-3">
                     <!-- Paper Style Toggle -->
                     <button id="btn-paper-style" class="p-2 bg-white rounded-xl shadow-sm border border-gray-200 hover:bg-gray-50 text-gray-500 hover:text-dark transition-all" title="${t('toggle_paper')}">
                ...
                `<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" /></svg>`
                     ...
                     <div class="flex items-center bg-white rounded-xl shadow-sm border border-gray-200 p-1">
                        <button id="btn-prev-page" class="p-1.5 text-gray-400 hover:text-primary hover:bg-gray-50 rounded-lg transition-colors ${activePageIndex === 0 ? 'opacity-30 cursor-not-allowed' : ''}" ${activePageIndex === 0 ? 'disabled' : ''} title="${t('prev_page')}">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                        </button>
                        <span class="text-xs font-bold text-gray-500 w-16 text-center select-none uppercase tracking-wider">${t('page_num')} ${activePageIndex + 1}</span>
                        <button id="btn-next-page" class="p-1.5 text-gray-400 hover:text-primary hover:bg-gray-50 rounded-lg transition-colors" title="${t('next_page')}">
                    ...
                    <div class="flex-1 overflow-hidden p-3 space-y-2" style="scrollbar-width: none;">
                        ${data.notebook.notes.map(note => `
                            <div class="note-item p-4 rounded-xl cursor-pointer transition-all border border-transparent ${note.id === activeNoteId ? 'bg-white shadow-md border-gray-100' : 'hover:bg-white/60 hover:border-gray-100 text-gray-500'}" data-id="${note.id}">
                                <h4 class="font-bold text-dark truncate mb-1">${note.title || t('untitled_note')}</h4>
                                <p class="text-xs text-gray-400 truncate">${stripHtml(note.pages ? note.pages[0] : (note.content || '')).substring(0, 40) || t('empty_note')}</p>
                                <div class="mt-2 text-[10px] text-gray-300 font-medium tracking-wide flex justify-between items-center">
                                   <span>${new Date(note.date).toLocaleDateString()}</span>
                                   ${note.id === activeNoteId ? `
                                   <button class="btn-delete-note p-1 text-gray-300 hover:text-primary transition-colors" title="${t('delete_page')}">
                ...
                <div class="flex-1 bg-white h-full flex flex-col relative z-0">
                    ...
                    'background-image: linear-gradient(#e5e7eb 1px, transparent 1px); background-size: 100% 2.5rem; margin-top: 3.5rem;'}">
                             ...
                             <div class="relative z-10 px-16 pt-14 pb-12 min-h-full">
                                  <!-- Title Input -->
                                  <input type="text" id="note-title-input" class="w-full text-4xl font-bold text-dark bg-transparent border-none focus:ring-0 placeholder-gray-300 mb-10 h-10 p-0 font-display leading-[2.5rem]" value="${activeNote.title}" placeholder="${t('page_title_placeholder')}">

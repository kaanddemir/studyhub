        ...
        if (currentImage) {
            element.innerHTML = `
                <div class="w-full h-full relative group overflow-hidden rounded-3xl bg-gray-100 flex items-center justify-center">
                    <img id="dashboard-photo-img" src="${currentImage}" class="w-full h-full object-cover pointer-events-none transition-transform duration-100 origin-center" style="transform: scale(${currentZoom})" alt="Dashboard Photo">
                    ...
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 z-10 transition-all duration-300">
                        <!-- Change Photo -->
                        <button id="btn-change-photo" class="p-3 bg-white text-dark hover:text-primary rounded-full hover:scale-110 transition-transform shadow-lg cursor-pointer" title="Change Photo">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                        </button>

                        <!-- Zoom Toggle -->
                        <button id="btn-toggle-zoom" class="p-3 bg-white text-dark hover:text-primary rounded-full hover:scale-110 transition-transform shadow-lg cursor-pointer" title="Zoom Image">
                        ...
                        <!-- Delete Photo -->
                         <button id="btn-delete-photo" class="p-3 bg-white text-dark hover:text-primary rounded-full hover:scale-110 transition-transform shadow-lg cursor-pointer" title="Remove Photo">
                    ...
                    <div id="photo-delete-modal" class="absolute inset-0 bg-white/90 backdrop-blur-sm z-30 flex flex-col items-center justify-center opacity-0 invisible transition-all duration-200">
                        ...
                        <h4 class="font-bold text-dark text-lg mb-1">Delete Photo?</h4>
                        <p class="text-xs text-gray-500 mb-4">This cannot be undone.</p>
                        <div class="flex gap-2">
                             <button id="cancel-delete-photo" class="px-4 py-2 rounded-xl bg-gray-100 text-gray-500 font-bold text-xs hover:bg-gray-200 transition-colors">Cancel</button>
                             <button id="confirm-delete-photo" class="px-4 py-2 rounded-xl bg-primary text-white font-bold text-xs hover:opacity-90 shadow-lg shadow-primary/30 transition-all">Delete</button>
                        </div>
                    </div>
                </div>
            ...
            element.innerHTML = `
                <div class="w-full h-full flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 transition-colors group rounded-3xl border-2 border-dashed border-gray-200 hover:border-primary/50" id="photo-placeholder">
                    ...
                    <p class="text-sm font-bold text-gray-500 group-hover:text-primary transition-colors">Add Photo</p>
                </div>
                <input type="file" id="photo-input" class="hidden" accept="image/*">
            `;
        }
                ...
                if (file) {
                    ...
                    reader.onload = (event) => {
                        ...
                        try {
                            ...
                            alert("Storage full. Cannot save image.");
                        }
                    };
